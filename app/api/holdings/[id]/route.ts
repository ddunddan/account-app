...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase
    .from('holdings')
    .update({ name: body.name, ticker: body.ticker, market: body.market, currency: body.currency, quantity: body.quantity, avg_price: body.avgPrice, current_price: body.currentPrice, price_updated_at: new Date().toISOString().split('T')[0], memo: body.memo })
    .eq('id', id)
    .select().single()
