...
export async function POST() {
  ...
  if (existing) {
    return NextResponse.json({
      id: existing.id, date: existing.date, totalAssets: existing.total_assets,
      totalLiabilities: existing.total_liabilities, netWorth: existing.net_worth,
      breakdown: { cash: existing.cash, stockKr: existing.stock_kr, stockUs: existing.stock_us, other: existing.other },
    })
  }
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  return NextResponse.json({
    id: data.id, date: data.date, totalAssets: data.total_assets, totalLiabilities: data.total_liabilities,
    netWorth: data.net_worth, breakdown: { cash: data.cash, stockKr: data.stock_kr, stockUs: data.stock_us, other: data.other },
  }, { status: 201 })
}
